...
export default class MarketNews extends Component {
        ...
        finnhubClient.companyNews(this.props.company, oneMonthAgo, today, (error, data, response) => {
            ...
            for (let item in data) {
                ...
                if (data[item].image.length > 3) {
                    article = data[item]
                    console.log('market news item: ', article)
                    break
                }
            }

            // this.setState({ finData: data[0], title: article.headline, description: article.summary, image: article.image, link: article.url })
            this.setState({ title: article.headline, description: article.summary, image: article.image, link: article.url })
        });
    }
    ...
    getFinNews = async () => {
        // let d = await finnhubClient.marketNews("general", {}, (error, data, response) => {
        //     console.log('respnose ', response)
        //     console.log('data ', data)
        //     return data
        //   });
        //   console.log('finn data = ', d)
        finnhubClient.marketNews("general", {}, (error, data, response) => {
            console.log('market news')
            console.log(data)
            let article
            let i = 0
            this.props.indexList.forEach((idx)=>{
                if(data[idx].image.length > 1){
                    article = data[idx]
                    i = idx
                    // break
                }
            })
            // for (i = 0; i < this.props.indexList; i++) {
            //     if (data[i].image) {
            //         article = data[i]
            ...
            this.setState({ finData: data[i], title: article.headline, description: article.summary, image: article.image, link: article.url })
        });
    }
